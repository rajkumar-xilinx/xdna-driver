 ...
 * Ported from test/amdxdna/ve2_hwctx.c (downstream reference).
 */
...
static void ve2_hq_kick_firmware(struct amdxdna_hwctx *hwctx)
{
	struct ve2_hwctx_link *link = hwctx->aux_ctx_priv;

	if (link && link->hal_ctx)
		ve2_aie_kick_cmd(link->hal_ctx);
}
...
{
	struct ve2_hwctx_priv *vp = ve2_hw_priv(hwctx);
	u32 slot = seq % vp->hsa_queue.hsa_queue_p->hq_header.capacity;
...
int ve2_hq_cmd_wait(struct amdxdna_hwctx *hwctx, u64 seq, u32 timeout_ms)
{
	...
	if (wait_jifs)
		ret = wait_event_interruptible_timeout(vp->waitq,
						       ve2_hq_read_index_done(hwctx, seq),
						       wait_jifs);
	else
	...
	mutex_unlock(&vp->hsa_queue.hq_lock);
	XDNA_DBG(xdna, "cmd_wait seq=%llu ret=%ld", seq, ret);
