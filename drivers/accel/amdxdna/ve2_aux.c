...
static int ve2_init(struct amdxdna_dev *xdna)
{
	...
	const struct amdxdna_hal_ops *hal_ops;
	int ret;
	...
	hal_ops = ve2_hal_ops_for_interface(xdna, fw_iface, &impl_type);
	if (!hal_ops) {
		XDNA_ERR(xdna, "No VE2 HAL backend for interface %s",
			 ve2_fw_interface_name(fw_iface));
		return -ENODEV;
	}
	...
	if (ret)
		return ret;
	XDNA_INFO(xdna, "VE2 HAL backend: %s (impl_type %u)",
		  ve2_fw_interface_name(fw_iface), impl_type);

	ret = amdxdna_hal_register_backend(AMDXDNA_PLATFORM_VE2, impl_type, hal_ops);
	if (ret)
		return ret;

	xdna_hdl->hal_dev = amdxdna_hal_create_device(NULL, xdna->ddev.dev,
						      AMDXDNA_PLATFORM_VE2);
	if (IS_ERR(xdna_hdl->hal_dev)) {
		ret = PTR_ERR(xdna_hdl->hal_dev);
		xdna_hdl->hal_dev = NULL;
		amdxdna_hal_unregister_backend(AMDXDNA_PLATFORM_VE2);
		XDNA_ERR(xdna, "HAL device create failed %d", ret);
		return ret;
	}
	amdxdna_hal_set_priv(xdna_hdl->hal_dev, xdna);
...
static void ve2_fini(struct amdxdna_dev *xdna)
{
	...
	if (hdl->hal_dev) {
		amdxdna_hal_destroy_device(hdl->hal_dev);
		hdl->hal_dev = NULL;
	}
...
{
	return ve2_hwctx_layer1_init(hwctx);
}

static void ve2_hwctx_fini(struct amdxdna_hwctx *hwctx)
{
	ve2_hwctx_layer1_fini(hwctx);
}
